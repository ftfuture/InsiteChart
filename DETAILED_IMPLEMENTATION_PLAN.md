...
본 계획은 68개의 구체적인 작업을 3개 우선순위로 분류하여 단계적으로 구현하기 위한 로드맵입니다.

---

## PRIORITY 1 - 즉시 필수 완료 (5개 Phase)
...
## Phase 3: CI/CD 파이프라인 재구축

**목표**: GitHub Actions 워크플로우 복구 및 완전한 자동화
**기한**: 2주
**현재 상태**: 최근 모든 워크플로우 제거됨

### 3.1 GitHub Actions 워크플로우 재구성

**파일 생성**: `.github/workflows/ci-pipeline.yml`
...
jobs:
  ...
  test:
    ...
    - name: Run unit tests
      env:
        DATABASE_URL: postgresql://postgres:postgres@localhost/insitechart_test
    ...
    - name: Run integration tests
      env:
        ...
        TESTING: "true"
      run: |
        pytest tests/integration -v --cov=backend --cov-report=append
  ...
  deploy-staging:
    ...
    - name: Deploy to Staging
      ...
      run: |
        ...
        # 배포 스크립트 실행
        ssh -i ~/.ssh/id_rsa $STAGING_USER@$STAGING_HOST << 'EOF'
          cd /app/insitechart
    ...
    - name: Smoke tests on Staging
      run: |
        pytest tests/smoke -v --staging-url=https://staging.insitechart.com
```

**추가 워크플로우**: `.github/workflows/security-scan.yml`
...
jobs:
  security:
    ...
    - name: Run Bandit (secret scanning)
...
**파일**: `.github/workflows/test-matrix.yml`
...
**Docker 멀티 아키텍처 빌드**: `.github/workflows/build-multiarch.yml`
...
**환경별 배포**: `.github/workflows/deploy-environments.yml`
    ...
    - name: Determine environment
      ...
      run: |
        if [ "${{ github.ref }}" = "refs/heads/develop" ]; then
          ...
          echo "kubeconfig=${{ secrets.DEV_KUBECONFIG }}" >> $GITHUB_OUTPUT
    ...
    - name: Deploy to ${{ steps.env.outputs.environment }}
      ...
      run: |
        ...
        kubectl set image deployment/backend \
          ...
          -n insitechart-${{ steps.env.outputs.environment }}
...
**파일**: `.github/workflows/deployment-monitoring.yml`
...
jobs:
  monitor:
    ...
    steps:
    - name: Wait for deployment stabilization (5 minutes)
      run: sleep 300

    - name: Check error rate
      id: error-rate
      run: |
        ERROR_RATE=$(curl -s "http://prometheus:9090/api/v1/query?query=rate(http_requests_total%7Bstatus%3D%22500%22%7D%5B1m%5D)" | jq '.data.result[0].value[1]' | tr -d '"')
        if (( $(echo "$ERROR_RATE > 0.05" | bc -l) )); then
          echo "error_rate=$ERROR_RATE" >> $GITHUB_OUTPUT
          echo "high_errors=true" >> $GITHUB_OUTPUT
        fi

    - name: Trigger rollback if error rate too high
      if: steps.error-rate.outputs.high_errors == 'true'
      run: |
        echo "Error rate: ${{ steps.error-rate.outputs.error_rate }}"
        echo "Triggering automatic rollback..."

        # Kubernetes 롤백
        kubectl rollout undo deployment/backend \
          -n insitechart-production
